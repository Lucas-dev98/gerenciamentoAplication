...
async function testNoticesCRUD() {
  ...
  try {
    ...
    const novoAviso = {
      ...
      content:
        'Este é um aviso de teste para verificar se o CRUD está funcionando corretamente. Conteúdo com mais de 10 caracteres.',
      type: 'info',
      priority: 'medium',
      isPinned: false,
    ...
    if (getOneResponse.ok) {
      ...
      console.log(
        '✅ Aviso encontrado:',
        getOneData.data?.notice?.title || 'Sem título'
      );
    } else {
    ...
    const avisoAtualizado = {
      ...
      content:
        'Este aviso foi atualizado para testar a funcionalidade de edição.',
      type: 'warning',
      priority: 'high',
      isPinned: true,
    };
    ...
    if (getAfterUpdateResponse.ok) {
      ...
      const avisoAtualizado = getAfterUpdateData.data?.notices?.find(
        (n) => n.id === avisoId
      );
      if (avisoAtualizado) {
        console.log('✅ Atualização confirmada:', avisoAtualizado.title);
        console.log(
          `   Novo tipo: ${avisoAtualizado.type}, Nova prioridade: ${avisoAtualizado.priority}`
        );
    ...
    if (deleteResponse.ok) {
      ...
      if (getAfterDeleteResponse.ok) {
        ...
        const avisoExcluido = getAfterDeleteData.data?.notices?.find(
          (n) => n.id === avisoId
        );
        if (!avisoExcluido) {
